# Funciones para la intefaz de consola
...
def registrar_mascota():
    ...
    precio = float(input("Precio del servicio por la mascota: "))

    if tipo == "perro":
        raza = input("Raza del perro: ")
        nivel_de_energia = input("Nivel de energía del perro: ")
        mascota = Perro(nombre, edad, salud, precio, raza, nivel_de_energia) # instanciamos la mascota perro y le pasamos la información
    elif tipo == "gato":
        raza = input("Raza del gato: ")
        independencia = input("Nivel de independencia del gato: ")
        mascota = Gato(nombre, edad, salud, precio, raza, independencia) # instanciamos la mascota gato y le pasamos la información
    else:
...
def registrar_venta(clientes, inventario):  # Para registrar una venta necesitamos tener a disposicion los clientes y el inventario
    nombre_cliente = input("Nombre del cliente: ")
    cliente = next((c for c in clientes if c.nombre == nombre_cliente), None) # Buscamos el nombre del cliente en clientes
    if not cliente:
...
def mostrar_menu():
    print("/n --- Menú de gestión de Patas felices ---")
...
def main():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            print(inventario.generar_alertas(umbral_minimo))
